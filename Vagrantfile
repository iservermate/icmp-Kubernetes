...
Vagrant.configure("2") do |config|
  ...
  (1.. NUM_MASTER_NODE).each do |i|
    config.vm.define "icmp-master0#{i}" do |node|
      ...
      node.vm.network :public_network, bridge: "en0: Ethernet", ip: IP_NW + "#{MASTER_IP_START + i}"
  ...
  (1.. NUM_WORKER_NODE).each do |i|
    config.vm.define "icmp-worker0#{i}" do |node|
      ...
      node.vm.network :public_network, bridge: "en0: Ethernet", ip: IP_NW + "#{NODE_IP_START + i}"
